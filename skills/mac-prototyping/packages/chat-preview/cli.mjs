...
const { values } = parseArgs({ options: {
  ...
  format: { type: 'string' }, 'max-bytes': { type: 'string' },
  help: { type: 'boolean' },
} });
if (values.help) {
  console.log('mac-chat-preview --entry preview.tsx --output /private/output/preview.html [--component] [--mac-chrome-directory toolkit] [--font /local/SF-Pro-Text-Regular.otf] [--config preview.config.json] [--format gzip|raw] [--dependencies project] [--toolchain project] [--additional-symbols names.json] [--python executable] [--asset-root public] [--root mac-chat-preview] [--max-bytes 1000000] [--local-symbols]');
} else {
  try {
    let config = {};
    if (values.config) {
