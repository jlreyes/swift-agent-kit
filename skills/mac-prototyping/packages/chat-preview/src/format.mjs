...
export function escapeRawText(text, tag) {
  return text.replace(new RegExp(`</${tag}`, 'gi'), match => '<\\/' + match.slice(2));
}
export function formatPreview({ root, css, script, format = 'gzip', maxBytes = 1_000_000 }) {
  ...
  if (format === 'gzip') {
    ...
    const bootstrap = `(async()=>{try{if(typeof DecompressionStream!=='function')throw new Error('This browser cannot decompress the preview. Request raw output.');const b=Uint8Array.from(atob('${payload}'),c=>c.charCodeAt(0));const p=JSON.parse(await new Response(new Blob([b]).stream().pipeThrough(new DecompressionStream('gzip'))).text());const s=document.createElement('style');s.textContent=p.css;document.head.append(s);const j=document.createElement('script');j.textContent=p.script;document.body.append(j);}catch(e){document.getElementById('${root}').textContent='Preview could not load: '+e.message;console.error(e);}})();`;
