...
- **Resizable by default** from all four edges and corners. `minSize` is the preferred floor; a smaller canvas wins so a positive, reachable frame remains even when the canvas is smaller than the normal safe insets. Interactive dragging may leave a window partially beyond the canvas or under the Dock. Dragging, resizing, and canvas-change handling use the nearest desktop canvas; when that canvas shrinks, the window retains a reachable draggable title area. Standalone windows apply the same reachability rule on viewport resize. Initial layout capture excludes caller-owned transform/translate/rotate/scale, so those effects are not baked into geometry and reapplied. An active gesture rebases when its canvas changes size. Set `resizable={false}` for intentionally fixed-size utility windows.
