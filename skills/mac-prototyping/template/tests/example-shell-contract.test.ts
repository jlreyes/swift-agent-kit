...
test("the example shell boots from the same immutable app definition it registers", () => {
  ...
  expect(exampleSource).toContain('kind: "systemSymbol"');
  expect(exampleSource).toContain('name: "face.smiling"');
  expect(exampleSource).not.toContain("/mac-assets/dock/finder.png");
